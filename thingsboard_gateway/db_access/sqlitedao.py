...
class SqliteDao:
    ...
    def __init__(self, db_path, single_threaded=False):
        self.conn = sqlite3.connect(db_path, check_same_thread=single_threaded, timeout=10)
    ...
    def insert_row(self, table_name, row_tuple):
        ...
        rt = 0
        try:
            rt = cursor.execute(query, values)
            self.conn.commit()
    ...
    def     updateRequest(self, table_name, update_dict, search_dict):
        if not update_dict:
            return
        if not search_dict:
            raise ValueError("must apply search conditions")
    ...
    def update_con(self, table_name, update_dict, search_dict):
        if not update_dict:
            return
        if not search_dict:
            raise ValueError("must apply search conditions")
        ...
        for k, v in update_dict.items():
            set_strings.append("{}=?".format(k))
            value_strings.append(str(v).replace("'", '"'))
        for k, v in search_dict.items():
            search_strings.append("{}=?".format(k))
            value_strings.append(v)
        query += ", ".join(set_strings) + " WHERE "
    ...
    def update_monitor(self, table_name, update_dict):
        if not update_dict:
            return
        query = "UPDATE {} SET ".format(table_name)
        set_strings = []
    ...
    def get_items(self, class_type, search_dict, order_by=None, limit=None, offset=None, desc=True):
        rows = self.search_table(class_type.TABLE_NAME, search_dict, order_by=order_by, limit=limit, offset=offset,
                                 desc=desc)
