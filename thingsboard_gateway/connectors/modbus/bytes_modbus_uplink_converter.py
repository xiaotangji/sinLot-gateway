...
class BytesModbusUplinkConverter(ModbusConverter):
    ...
    def __parse_response_data(self, tag, response, endian_order, word_endian_order):
        decoded_data = None
        if not isinstance(response, ModbusIOException) and not isinstance(response, ExceptionResponse):
            if tag["functionCode"] in [1, 2]:
                result = response.bits
                result = result if endian_order == Endian.Little else result[::-1]
                log.debug(result)
                if tag["type"].lower() == "bits":
                    decoded_data = result[:tag.get("objectsCount")]
                    if len(decoded_data) == 1 and isinstance(decoded_data, list):
                        decoded_data = decoded_data[0]
                else:
                    decoded_data = result[0]
            elif tag["functionCode"] in [3, 4]:
                decoder = None
                registers = response.registers
                log.debug("Tag: %s Config: %s registers: %s", tag['tag'], str(tag), str(registers))
                try:
                    ...
                    decoder = BinaryPayloadDecoder.fromRegisters(registers, endian=endian_order,
                                                                 wordorder=word_endian_order)
                assert decoder is not None
                decoded_data = self.__decode_from_registers(decoder, tag)
                if tag.get("divider"):
                    decoded_data = float(decoded_data) / float(tag["divider"])
                if tag.get("multiplier"):
                    decoded_data = decoded_data * tag["multiplier"]
        else:
            log.exception(response)
            decoded_data = None
        return decoded_data
    ...
    def bytes_to_raw_data(self, tag, response):
        if tag.get("byteOrder"):
            byte_order = tag["byteOrder"]
        else:
            byte_order = "BIG"
        if tag.get("wordOrder"):
            word_order = tag["wordOrder"]
        else:
            word_order = "LITTLE"

        endian_order = Endian.Little if byte_order.upper() == "LITTLE" else Endian.Big
        word_endian_order = Endian.Little if word_order.upper() == "LITTLE" else Endian.Big
        decoded_data = self.__parse_response_data(tag, response, endian_order, word_endian_order)
        log.debug("bytes_to_raw_data:  key: %s \t value: %s", tag['tag'], str(decoded_data))
        rt = {tag['tag']: decoded_data}
        return rt

    def raw_data_to_bytes(self, tag):
        pass
