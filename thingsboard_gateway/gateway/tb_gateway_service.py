...
# from thingsboard_gateway.db_access.sqlite_helper import SqliteHelper
...
DEFAULT_CONNECTORS = {
            ...
            "rest": "RESTConnector",
            "snmp": "SNMPConnector",
            "mitsubishi": "MitsubishiPlcConnector",
        }
...
class TBGatewayService:
    ...
    def __init__(self, config_file=None):
        ...
        self.__db_path = self.__config['db'].get('datafile')
        global_var.set_db_path(self.__db_path)
        # collect_api_service.addConnector = self._add_connector
        # collect_api_service.reloadDeviceVariable = self.reload_device_variable
    ...
    def __new__(cls, *args, **kwargs):
        if not hasattr(TBGatewayService, "_instance"):
            with TBGatewayService._instance_lock:
                if not hasattr(TBGatewayService, "_instance"):
                    TBGatewayService._instance = object.__new__(cls)
                    TBGatewayService._instance.__initialized = False
        return TBGatewayService._instance
    ...
    def _load_connectors(self):
        if self.__config.get("use_connectors_from_db"):
            self._load_connectors_from_db()
        else:
            self._load_connectors_from_yaml()

    def read_single_tag(self, tag):
        rt = {}
        device_dao = DeviceDao()
        connect_name = device_dao.get_connector_name(tag['deviceName'])
        if self.available_connectors.get(connect_name):
            rt = self.available_connectors[connect_name].read_single_tag(tag)
        else:
            rt['errorCode'] = 2
            rt['errorMsg'] = "not found connector"
        return rt
    ...
    def write_single_tag(self, tag):
        ...
        connect_name = device_dao.get_connector_name(tag['deviceName'])
        if self.available_connectors.get(connect_name):
            rt = self.available_connectors[connect_name].write_single_tag(tag)
        else:
            rt['errorCode'] = 2
            rt['errorMsg'] = "not found connector"
        return rt
    ...
    # 重新加载当前设备的所有变量
    def reload_device_variable(self, connector_name, device_name):
        if self.available_connectors.get(connector_name):
            self.available_connectors[connector_name].reload_device_variable(device_name)

    def remove_device(self, connector_name, device_name):
        if self.available_connectors.get(connector_name):
            self.available_connectors[connector_name].remove_device(device_name)

    def add_connector(self, connect_row):
        connector_type, connector_config = self._load_connector_to_config(connect_row)
        self._connect_with_single_connector(connector_type, connector_config)

    def update_connector(self, connect_row):
        connector_name = connect_row['name']
        self.remove_connector(connector_name)
        self.add_connector(connect_row)

    def remove_connector(self, connector_name):
        if self.available_connectors.get(connector_name):
            self.available_connectors[connector_name].close()
            del self.available_connectors[connector_name]

    def _load_connector_to_config(self, connector_row):
        connector_type = None
        connector_config = {}
        try:
            if connector_row["enable"]:
                connector_class = TBModuleLoader.import_module(connector_row["protocol_type"],
                                                               self._default_connectors.get(connector_row["protocol_type"],
                                                                                            connector_row.get("class")))
                self._implemented_connectors[connector_row["protocol_type"]] = connector_class
                if isinstance(connector_row["json"], dict):
                    connector_conf = connector_row["json"]
                else:
                    connector_conf = json.loads(connector_row["json"])

                # if not self.connectors_configs.get(connector_row['protocol_type']):
                #     self.connectors_configs[connector_row['protocol_type']] = []
                connector_conf["name"] = connector_row["name"]
                connector_config = {"name": connector_row["name"], "config": {"json": connector_conf}}
                # self.connectors_configs[connector_row['protocol_type']].append(connector_config)
                connector_type = connector_row['protocol_type']
        except Exception as e:
    ...
    def _load_connectors_from_db(self):
        self.connectors_configs = {}
        if self.__config.get("db"):
            connect_dao = ConnectorDao()
            rows = connect_dao.get_all()
            for connector in rows:
                connector_type, connector_config = self._load_connector_to_config(connector)
                if not connector_type:
                    continue
                if not self.connectors_configs.get(connector_type):
                    self.connectors_configs[connector_type] = []
                self.connectors_configs[connector_type].append(connector_config)

    def _load_connectors_from_yaml(self):
        self.connectors_configs = {}
    ...
    def _connect_with_single_connector(self, connector_type, connector_config):
    ...
    def save_to_db(self, connector_name, raw_data):
        device_dao = DeviceDao()
        device_variable_dao = DeviceVariableDao()
        device_id = device_dao.get_device_id(raw_data["deviceName"])
        device_variable_dao.update_variable_value(device_id, raw_data["telemetry"])
        device_variable_dao.update_variable_value(device_id, raw_data["attributes"])
